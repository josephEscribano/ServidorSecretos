...
public class EncriptacionAsimetrica {
    ...
    public boolean crearKeyStore(Usuario usuario, PrivateKey clavePrivada, X509Certificate cert)  {
        boolean confirmar = false;
        try {
            //Creamos el keyStore
            KeyStore ks = KeyStore.getInstance(ConstantesEncriptacion.PKCS_12);
            char[] password = usuario.getPass().toCharArray();
            ks.load(null, null) ;
            ks.setCertificateEntry(ConstantesEncriptacion.PUBLICA, cert);
            ks.setKeyEntry(ConstantesEncriptacion.PRIVADA, clavePrivada, usuario.getPass().toCharArray(), new Certificate[]{cert});
            FileOutputStream fos = new FileOutputStream(Constantes.RUTA_CLAVES + usuario.getUserName() + Constantes.KEYSTORE_PFX);
    ...
    public Either<String,String> crearCerfiticado(String userName, PublicKey publicKeyUsuario,PrivateKey privateKeyServer) {
        ...
        try{
            ...
            X509Certificate cert = certificado.generate(privateKeyServer);
    ...
    public Either<String, String> descifrar(String passUser, String passDesencriptar,InputStream ruta) {
        ...
        try {
            Cipher cifrador = Cipher.getInstance(ConstantesEncriptacion.RSA);
            Either<String, PrivateKey> result = getPrivateKey(passUser,ruta);
            if (result.isRight()) {
